...
onAuthStateChanged(auth,async u=>{if(!u||u.isAnonymous){UI.$("#profile").innerHTML=`<div class="empty-state"><a class="link" href="login.html">Entre</a> para editar seu perfil.</div>`;UI.$("#support").innerHTML=`<div class="empty-state"><a class="link" href="login.html">Entre</a> para configurar apoio.</div>`;return}const [us,as]=await Promise.all([getDoc(doc(db,"usuarios",u.uid)),getDoc(doc(db,"autores",u.uid))]);const p=us.exists()?us.data():{},a=as.exists()?as.data():{};UI.$("#settingsName").value=p.nome||u.displayName||"";UI.$("#settingsBio").value=p.biografia||"";const social=a.redesSociais||p.redesSociais||{};UI.$("#socialInstagram").value=socialHandle(social.instagram,"instagram");UI.$("#socialTikTok").value=socialHandle(social.tiktok,"tiktok");UI.$("#socialYouTube").value=socialHandle(social.youtube,"youtube");UI.$("#socialTwitter").value=socialHandle(social.twitter,"twitter");UI.$("#socialFacebook").value=socialHandle(social.facebook,"facebook");UI.$("#socialSite").value=social.site||"";UI.$("#pixKey").value=a.pixChave||"";UI.$("#pixName").value=a.pixNome||"";UI.$("#supportUrl").value=a.apoioUrl||"";UI.$("#supportMessage").value=a.apoioMensagem||"";
 UI.$("#profileForm").onsubmit=async e=>{e.preventDefault();const name=UI.$("#settingsName").value.trim(),bio=UI.$("#settingsBio").value.trim();const socialLinks={instagram:socialLink(UI.$("#socialInstagram").value,"instagram"),tiktok:socialLink(UI.$("#socialTikTok").value,"tiktok"),youtube:socialLink(UI.$("#socialYouTube").value,"youtube"),twitter:socialLink(UI.$("#socialTwitter").value,"twitter"),facebook:socialLink(UI.$("#socialFacebook").value,"facebook"),site:UI.$("#socialSite").value.trim()};if(socialLinks.site&&!UI.safeHttps(socialLinks.site))return UI.toast("Use um link HTTPS válido no site / portfólio.");let photo=p.fotoURL||u.photoURL||"",file=UI.$("#settingsPhoto").files[0];try{if(file){UI.toast("Enviando foto...");photo=await uploadImage(file,{kind:"avatar"})}await setDoc(doc(db,"usuarios",u.uid),{uid:u.uid,nome:name,biografia:bio,fotoURL:photo,email:u.email||"",redesSociais:socialLinks,atualizadoEm:serverTimestamp()},{merge:true});await setDoc(doc(db,"autores",u.uid),{uid:u.uid,nome:name,biografia:bio,fotoURL:photo,redesSociais:socialLinks,atualizadoEm:serverTimestamp()},{merge:true});await updateProfile(u,{displayName:name,photoURL:photo||null});UI.toast("Perfil atualizado")}catch(err){console.error(err);UI.toast(err.message||"Não foi possível salvar")}};
