document.addEventListener("DOMContentLoaded", () => {
  ...
  hero.innerHTML = `<div class="loading-state">Carregando histórias do Firebase...</div>`;
  ...
  async function carregar() {
    try {
      ...
      if (!books.length) {
        hero.innerHTML = `<div class="loading-state">Nenhum livro publicado foi encontrado no Firebase.</div>`;
        grid.innerHTML = `<div class="loading-state">Publique um livro para ele aparecer aqui.</div>`;
        renderContinue([]);
        return;
      }
      ...
      hero.innerHTML = `
        <div class="loading-state">
          Não foi possível carregar os livros do Firebase.<br>
          <span class="small">${UI.esc(erro?.message || "Erro de conexão")}</span>
        </div>`;
      grid.innerHTML = `<div class="loading-state">A conexão com o Firestore precisa ser corrigida.</div>`;
      renderContinue([]);
    }
  }
